...
public:
	...
	static self_type * value(map_iterator x)
	{
		return *x;
	};
...
public:
	...
	void     set_value(const ValueT & x)
	{
		value.reset(new ValueT(x));
	};
	void     clr_value()
	{
		set_value(nullptr);
	...
	void swap_value(ValueHolder & other)
	{
...
public:
	...
	void     set_value(const value_type & x)
	{
		count = x;
	};
	void     clr_value()
...
public:
	...
	bool starts_with(AtomT x) const
	{
		return (*chunk)[begin] == x;
	};
	key_iterator kbegin() const
	{
		return std::addressof((*chunk)[begin]);
	};
	key_iterator kend()   const
	{
		return std::addressof((*chunk)[end]);
	};

	ChunkT * insertion_hint()
	{
		return chunk;
	}
...
private:
	...
	NodeT * root()
	{
		return std::addressof(*edges.begin());
	}
...
public:
	...
	void add(KeyIterator it, KeyIterator end, const value_type & value)
	{
		return insert(it, end, value,
			      [] (value_type & old, const value_type & n) {
				old += n;
			});
	}
...
private:
	...
	{
		exactMatch = false;
		return find_prefix_int(root_node, it, kend, [&exactMatch] () {
				exactMatch = true;
			});
	}
